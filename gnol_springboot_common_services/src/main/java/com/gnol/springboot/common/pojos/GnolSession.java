...
import com.gnol.plugins.core.context.SpringContextHolder;
 ...
 */
public abstract class GnolSession implements Serializable {
    ...
    public GnolSession(String sessionId, long visitorId, String visitorName, int userType, String source,
            long timeout) {
        if (visitorId <= 0) {
            throw new GnolRuntimeException("SESSION 创建失败！");
        }
        ...
        if (getUserTypeEnum() == null) {
            throw new GnolRuntimeException("SESSION 创建失败，用户类型不存在！");
        }
    ...
    public boolean equals(Object obj) {
        ...
        if (obj instanceof GnolSession) {
            GnolSession other = (GnolSession) obj;
