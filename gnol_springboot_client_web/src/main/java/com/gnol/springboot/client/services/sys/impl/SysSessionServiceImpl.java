...
import com.gnol.springboot.client.daos.sys.ExtSysSessionDao;
 ...
 * @Description: gnol 系统_用户详情权限临时表，用户在登录，下线等情况下触发插入、删除、更新操作，然后以此表关联业务表查询指定范围内的 Service 实现
 */
...
public class SysSessionServiceImpl extends AbstractBaseService<BaseSysSessionDao, SysSession, Long>
    ...
    /**
     * gnol 系统_用户详情权限临时表，用户在登录，下线等情况下触发插入、删除、更新操作，然后以此表关联业务表查询指定范围内的扩展 Dao
     */
    @Resource(name = "extSysSessionDao")
    private ExtSysSessionDao sysSessionDao;
    /**
     * gnol 系统角色表 Service 实现
     */
    ...
    public SysSession updateUserAddress(SysSession session) {
        ...
        // 将用户对象插入 gnol 系统_用户详情权限临时表，用户在登录，下线等情况下触发插入、删除、更新操作，然后以此表关联业务表查询指定范围内的 pojo
