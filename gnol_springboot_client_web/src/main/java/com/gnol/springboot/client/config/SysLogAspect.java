...
import com.gnol.plugins.net.tools.UserAgentUtil;
import com.gnol.plugins.tools.json.SFJsonUtil;
import com.gnol.springboot.client.services.sys.SysLogBatchService;
...
public class SysLogAspect {
     ...
     * 系统批量日志服务
     */
    @Resource(name = "sysLogBatchServiceImpl")
    private SysLogBatchService sysLogBatchService;

    @Pointcut("execution(* com.gnol.springboot.client.controllers..*Controller.*(..))")
    // @Pointcut("execution(* com.gnol.springboot.client.controllers.*.*Controller.*(..))")
    ...
    public void doLog(JoinPoint joinPoint, Throwable throwable) {
        ...
        sysLogBatchService.insertSysLog(log, joinPoint.getSignature().getName());
        logger.debug("插入日志成功...");
    }
