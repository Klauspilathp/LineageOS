...
import com.gnol.springboot.client.daos.sys.ExtSysOrgDao;
import com.gnol.springboot.client.daos.sys.ExtSysUserDao;
import com.gnol.springboot.client.dtos.tree.SelectTree;
import com.gnol.springboot.client.dtos.tree.SelectTreeUtil;
 ...
 * @Description: gnol 系统组织机构 Service 实现
 */
    ...
    public List<PageData> listPDBy(PageData pd) {
        ...
        if (session_userId.equals(GnolConstant.SUPER_ADMIN_ID)) {
            ...
            if (org == null || org.isEmpty()) {
                throw new GnolRuntimeException("当前登录用户组织机构信息不存在！");
            }
            // 当前登录用户组织机构编号
            Long session_orgId = org.getLong("orgId");
            if (session_orgId == null) {
                throw new GnolRuntimeException("当前用户组织机构编号为空！");
            }
            LevelEnum levelEnum = LevelEnum.forKey(org.get("level"));
            if (levelEnum == null) {
                throw new GnolRuntimeException("当前用户组织机构级别为空！");
            }
            // 查询类型
            ...
            if (parentId == null || parentId.compareTo(0L) != 1) { // 首次进入查询页
                ...
                switch (levelEnum) {
                    ...
                    case DEPARTMENT:
                        break;
                    default:
                        throw new GnolRuntimeException("当前用户组织机构级别有误！");
                }
            }
        }
