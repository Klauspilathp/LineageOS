package com.d7c.springboot.common.configurations.exception;
...
import com.d7c.plugins.core.exception.D7cException;
import com.d7c.plugins.core.exception.D7cRuntimeException;

/**
 * @Title: D7cExceptionHandler
 * @Package: com.d7c.springboot.common.configurations.exception
 * @author: 吴佳隆
...
@ConditionalOnProperty( // 存在对应配置信息时初始化该配置类
        prefix = "d7c.exception", // 配置前缀 d7c.exception
        name = "enabled", // 配置名称
        havingValue = "true", // 与配置值比较，如果相等则配置生效
        matchIfMissing = false // 默认配置值，即默认不开启此配置
)
@RestControllerAdvice
public class D7cExceptionHandler {

    @ExceptionHandler(D7cRuntimeException.class)
    public PageResult handlerD7cRuntimeException(D7cRuntimeException e) {
    ...
    public PageResult handlerException(Exception e) {
        ...
        } else if (e instanceof D7cException) { // d7c 系统抛出的需要处理异常定义基类
            model.put("exception", e.getClass().getName());
            message = e.getMessage();
            status = ((D7cException) e).getHs().getKey();
        } else {
            model.put("exception", e.getClass().getName());
