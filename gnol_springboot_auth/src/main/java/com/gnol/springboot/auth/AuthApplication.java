...
public class AuthApplication {
     ...
     * 本项目有两套认证方式：
     * 第一套：
     *    注释掉 com.gnol.springboot.auth.config.ClusterWebSecurityConfiguration 类上的注解，
     * 采用 com.gnol.springboot.auth.controllers.AuthController 里的三个控制方法实现请求的认证、校验和注销等。
     * 第二套：
     *    直接打开 com.gnol.springboot.auth.config.ClusterWebSecurityConfiguration 类上的注解，利用 security 内部的 login、logout 控制方法实现
     * 请求的认证、校验和注销等。
